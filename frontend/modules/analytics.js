...
(function () {
  ...
  function flush(keepRunning) { if (!currentScreen || !startedAt) return; const duration = Date.now() - startedAt; if (duration > 500) send('screen_active_time', { screen: currentScreen, active_duration_ms: Math.min(duration, INACTIVE_MS) }); startedAt = keepRunning === true ? Date.now() : 0; }
  ...
  window.setInterval(() => { if (Date.now() - lastActivity < INACTIVE_MS && currentScreen) { flush(true); send('session_heartbeat', { screen: currentScreen }); } }, 30000);
