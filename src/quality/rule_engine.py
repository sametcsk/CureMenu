...
def profile_hard_avoid_ingredients(profile: Dict[str, Any]) -> List[str]:
    ...
    Thin wrapper over the central food-constraint layer so there is a single source
    of truth. Raw disease names are never returned; raw allergy strings become
    hard-avoid only when the registry/catalog resolves them to a real food.
    """
    from src.quality.food_constraints import resolve_food_constraints  # lazy: avoid import cycle
